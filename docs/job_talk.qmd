...
title-slide-attributes:
    data-background-image: figs/qr-tinyurl-com.png
...
format:
  revealjs:
    ...
    footer: tinyurl.com/koff-thesis
code: 
  echo: false
  cache: true 
editor_options: 
...
plot_list = read_rds(here::here("docs", "data", "well_pred.rds"))
plot_list2 = read_rds(here::here("docs", "data", "poor_pred.rds"))
```
...
- What else can we learn from walking patterns in free-living data? 
...
- <span style="color:gray;"> What else can we learn from walking patterns in free-living data? </span>
...
+ Obtain joint distribution of acceleration and lag acceleration for a series of lags (image) 

+ Calculate scalar summaries of the joint distribution 
  
+ Time series $\rightarrow$ complex object (image) $\rightarrow$ scalar predictors 

+ Intuition: walking is cyclic process, we want to leverage dynamics of this process

+ We will walk through process for one second and one lag 
</div>
...
p1 = df %>%
  ...
  geom_point(aes(group = time), size = 3.5, color = "#4E97E0") +
...
p1 = df %>%
  ggplot(aes(x = vm, y = lag_vm)) +
  annotate(geom = "point", x = 0.622, y = 1.26, size = 3.5, color = "#4E97E0") +
  annotate(geom = "point", x = 0.613, y = 1.19, size = 3.5, color = "#F56600") +
...
p1 = df %>%
  ...
  geom_point(data = blue_pts, aes(group = time), color = "#4E97E0", size = 3.5) +
  geom_point(data = or_pts, aes(group = time), color = "#F56600", size = 3.5) +
  scale_x_continuous(limits = c(0.4, 2.6)) +
  scale_y_continuous(limits = c(0.4, 2.6)) +
  labs(x = "Acceleration (g)", y = "Lagged acceleration (g)") +
  theme(panel.grid = element_blank()) +
...
p2 = ggplot(pred_matrix, aes(x = col, y = row, fill = z_plot)) + 
  ...
  theme(legend.position = "none") + 
  scale_y_reverse() +
  geom_tile(data = subset(pred_matrix, is.infinite(n)), fill = "#FFFFFF")
```
...
pred_df %>% 
  ...
  scale_color_manual(values = c("#A45C98", "#008767"),
...
sample_dat %>% 
  ggplot(aes(x, y)) +
  geom_point(alpha = .6) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 1, color = "#002D72") +
  labs(x = "Predictor (X)", y = "Outcome (Y)") +
  annotate(geom = "label", 
           label = "hat(beta) == 1.9", parse = TRUE,
           x = 2, y = 0, color = "#002D72", size = 5) 
...
Sometimes our predictor is a series of values over time rather than a single value 

We can think of this as having many correlated predictors: 

$$Y_i = \alpha + \beta_1X_i(t_1) + \beta_2X_i(t_2) + \dots + \beta_TX_i(t_T) + \epsilon_i$$
...
p1 = ggplot(dfX, aes(t, X, color = id)) +
  ...
  scale_color_manual(values = c( "#4E97E0", "#CBA052", "#A45C98"),
...
p1 = dfX %>% filter(id == "subj1") %>% 
  ggplot(aes(t, X, color = id)) +
  geom_point(size = .9) +
  # geom_line(linewidth = 1.2, alpha = 0.9) +
  labs(title = "Data from Subject 1", y = expression(X[1](t)), x = "Time (t)") +
  scale_color_manual(values = c("#4E97E0")) +
  theme(legend.position = "none",
        legend.background = element_rect(fill = "white",color = "white"),
        legend.text = element_text(size = 14),
        legend.title = element_text(size = 15)) +
...
p2 = ggplot(beta, aes(t, beta)) +
  geom_line(linewidth = 1.3, alpha = 0.9, color = "#008767") +
  labs(title =expression(Coefficient ~ Function ~ hat(beta)(t)), y = "Value", x = "Time (t)") +
  geom_hline(aes(yintercept = 0), linetype = "dashed")+
  annotate(geom = "label", x = 0.25, y = 0.5, label = "Weight for time t = 0.25", size = 5, color = "#444444", vjust = 1) +
...
p3 = ggplot(dfProd, aes(t, prod)) +
  geom_area(alpha = 0.35, fill = "#4E97E0") +
  geom_line(linewidth = 1.2, color = "#4E97E0") +
  # labs(title = expression(Contribution ~ from ~ Subject ~ 1: ~  X[1]~(t)~x~ hat(β)~(t)),, y = "Value", x = "Time (t)")
  labs(title = expression(Pointwise ~ contributions: ~ X[1](t) ~ times ~ hat(beta)(t)),
       y = "Value", x = "Time (t)")

p3_bar = ggplot(dfProd, aes(t, prod)) + 
  geom_col(width = diff(range(t)) / length(t), fill = "#4E97E0", alpha = 0.8, color = "#444444") +
  labs(title = expression(Pointwise ~ contributions: ~ X[1](t) ~ x ~ hat(beta)(t) ~ x ~ Delta ~t),
       y = "Value", x = "Time (t)")

dt = diff(range(t)) / (length(t) - 1)
...
p3_bar = ggplot(dfProd, aes(t, prod)) + 
  ...
  labs(
    # title = expression(Riemann ~ sum: ~ sum(X[1](t[k])~hat(beta)(t[k])~Delta~t)),
    title = "Contribution to outcome",
    subtitle = "Each bar = one term in the sum",
    y = expression(X[1](t)~hat(beta)(t)),
    x = "Time (t)"
  ) 
ymin = min(dfProd$prod, 0)

p3_bar = p3_bar +
  geom_segment(
    x = min(t), xend = max(t),
    y = ymin - 0.1 * diff(range(dfProd$prod)),
    yend = ymin - 0.1 * diff(range(dfProd$prod)),
    linewidth = 0.8,
    color = "#444444"
  ) +
  geom_segment(
    x = min(t), xend = min(t),
    y = ymin - 0.08 * diff(range(dfProd$prod)),
    yend = ymin - 0.12 * diff(range(dfProd$prod)),
    linewidth = 0.8,
    color = "#444444"
  ) +
  geom_segment(
  ...
  annotate(
    "text",
    x = mean(t),
    y = ymin - 0.18 * diff(range(dfProd$prod)),
    label = expression(paste("Total contribution  ", approx, "  ", sum(X(t[k])~beta(t[k])~Delta~t))),
    size = 5,
    color = "#444444"
  )

highlight_idx = c(30)
df_highlight = dfProd %>% slice(highlight_idx)
...
p3_bar = p3_bar +
  ...
  annotate(
    "label",
    x = df_highlight$t[1],
    y = df_highlight$prod[1] + 0.1 * diff(range(dfProd$prod)),
    label = expression(X(t[k])~beta(t[k])~Delta~t),
    size = 4,
    color = "#CBA052",
    hjust = 0
  )
...
Instead of estimating a separate coefficient at each time point:

$$
Y_i = \alpha + \sum_{k=1}^T \beta_k X_i(t_k) + \epsilon_i
$$

We model the coefficients as a **smooth function of time**:

$$
Y_i \approx \alpha + \sum_{k=1}^T X_i(t_k)\,\beta(t_k)\,\Delta t + \epsilon_i
$$

(**Riemann sum** approximation; in the limit, this becomes an integral)
...
::: {.notes}
X_i(t_k): value of curve at t_k
beta(t_k): weight at time t_k
delta t: width of each time bin
:::
...
pp = dd1 %>% 
  mutate(lag = "Lag = 0.01s") %>% 
  bind_rows(dd2 %>% mutate(lag = "Lag = 0.02s")) %>% 
  bind_rows(dd50 %>% mutate(lag = "Lag = 0.50s")) %>% 
  bind_rows(dd99 %>% mutate(lag = "Lag = 0.99s")) %>% 
  ggplot(aes(x=vm, y=lag_vm, color = density)) +
  geom_point(size = 0.9, alpha = 0.8) +
  facet_grid(.~lag) +
  scale_color_viridis(name = "Density", option = "B") +
...
plot_ly(
  ...
  type = "scatter3d",
  mode = "markers",
  marker = list(size = 1, opacity = 0.5, color = "black")
) %>%
  layout(
    margin = list(l = 80, r = 80, b = 80, t = 0), 
    title = list(text = "Predictors for one person and one second", y = 0.95),
    scene = list(
...
+ Train and test on session 1
  + Logistic regression w/ grid cells: 92 
  + <span style="color:#008767;">Functional regression: 98 </span>

+ Train on session 1, test on session 2 
  + Logistic regression w/ grid cells: 41 
  + <span style="color:#008767;">Functional regression: 53 </span>
...
+ Train and test on session 1
  + Logistic regression w/ grid cells: 92 
  + <span style="color:#008767;">Functional regression: 98 </span>
  + XGBoost w/ grid cells: 93 

+ Train on session 1, test on session 2 
  + Logistic regression w/ grid cells: 41 
  + Functional regression: 53 
  + <span style="color:#008767;">XGBoost w/ grid cells: 58</span>
...
- 3:1 train/test split 
- Logistic regression + weighting to overcome class imbalance
- <span style="color:#008767;">43\%</span> rank-1 accuracy 
- <span style="color:#008767;">73\%</span> rank-5 accuracy
- <span style="color:#008767;">97\%</span> rank-1\% accuracy (correct subject is in the top 130 predictions)
- <span style="color:#008767;">100\%</span> rank-5\% accuracy (correct subject is in the top 650 predictions)
...
p2 = plot_list[[1]] + plot_list[[2]] + plot_list[[3]]  +
...
Steps are more predictive of mortality than age, sex, or BMI 
...
dose_resp %>%
  ...
  theme(legend.position = "none") +
  geom_rug(data = rug_dat, aes(x = value), sides = "b", color = "#F56600", size = 0.5, alpha = 0.5) +
  ...
  annotate(geom = "curve", x = 10000, xend = 8000, y = 1.6, yend = 0.9, curvature = -0.2,
    ...
    color = "#444444",
    linewidth = .5) +
  annotate(geom = "label", x = 2000, y = 0.1, color = "#F56600", label = "Mortality events", hjust = 0) 
```
...
steps_summ %>% 
  ...
  scale_color_manual(values = c("#A45C98", "#4E97E0"), name = "") +
  theme(legend.position = "bottom",
...
+ Fit separate GLM at each point $s$, smooth the resulting point estimates 
...
p1 = res_g %>% 
  ...
  theme(palette.color.discrete = c("#CBA052", "#4E97E0", "#008767FF")) +
...
p2 = res_g %>% 
  ...
  geom_hline(aes(yintercept = .95), color = "#A45C98", linetype = "dashed")
...
p1 = df2 %>% 
  ...
  scale_color_manual(values = c("#CBA052", "#008767FF"), name = "Model") +
  scale_fill_manual(values = c("#CBA052", "#008767FF"), name = "Model") +
  theme(legend.position = "bottom") +
  geom_ribbon(aes(x = s, ymin = lower, ymax = upper, fill = model), alpha = 0)
p1
```
...
p1 + 
  ...
  geom_line(aes(color = model), linewidth = 1.2)  +
  scale_fill_manual(values = c("#CBA052", "#008767FF"), name = "Model",
                    labels = c("Unweighted", "Weighted w/ BRR")) +
  scale_color_manual(values = c("#CBA052", "#008767FF"), name = "Model",
                    labels = c("Unweighted", "Weighted w/ BRR")) 
...
::: {.incremental}
+ Applying fingerprinting methods in other data sources (e.g. arterial waveform)
...
![](figs/collaborators2.png)
---

### JHU Biostatistics Faculty, Students, Staff 
...
**Hallway conversations and life advice** 

+ Margaret Taub, PhD
+ Elizabeth Colantuoni, PhD, ScM
+ John McGready, PhD, ScM
...
- Link to slides: [tinyurl.com/koff-thesis](tinyurl.com/koff-thesis) 
...
![](figs/qr-tinyurl-com_nurl.png){width=35%}
...
step_acc %>% 
  ...
  scale_fill_manual(values =c( "#CBA052", "#4E97E0", "#008767", "#A45C98")) +
  theme(legend.position = "none") + 
...
xdf %>% 
  ...
  facet_grid(.~age_cat) +
  scale_fill_manual(values =c( "#CBA052", "#4E97E0", "#008767", "#A45C98")) +
  scale_color_manual(values =c( "#CBA052", "#4E97E0", "#008767", "#A45C98")) +
  scale_x_continuous(limits=c(0,25000),
                     labels = seq(0, 30, 5),
                     breaks = seq(0, 30000, 5000))  +
...
### Survey function on scalar regression: simulation details
...
### Fingerprinting with other data sources: arterial waveform 
